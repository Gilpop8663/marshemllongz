...
export default function CartList({ cartItems }: CartListProps) {
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-7 items-center  sm:border-t  sm:border-b border-primaryBlack ">
        <div className="w-full sm:col-span-6">
          {cartItems.map((cartItem) => (
            <CartItem key={cartItem.id} cart={cartItem} />
          ))}
        </div>
        <div className="flex flex-col gap-2 text-sm text-center justify-center px-4 py-8">
          <span>무료</span>

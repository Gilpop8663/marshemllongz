...
describe('페이지 버튼을 눌러 공지 사항 리스트를 불러오는 지 확인한다.', () => {
  ...
  test.each([
    ...
    async (currentPage, size, expected) => {
  ...
  test.each([
    ...
    '현재 페이지가 %s이고, 이전의 페이지를 불러올 때 시작 페이지 숫자는 현재 시작 페이지 - 5을 한 값 %s이다.',
    async (currentPage, expected) => {
      const { result } = renderHook(() => usePagination(currentPage, 5));
